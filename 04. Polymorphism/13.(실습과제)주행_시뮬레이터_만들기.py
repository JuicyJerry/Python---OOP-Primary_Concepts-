...
class DrivingSimulator:
    ...
    def add_vehicle(self, new_vehicle):
        ...
        if isinstance(new_vehicle, Vehicle):
            self.new_vehicles.append(new_vehicle)
        else:
            #return "{}은 교통 수단이 아니기 때문에 추가할 수 없습니다.".format(new_vehicle)
            print("{}은 교통 수단이 아니기 때문에 추가할 수 없습니다.".format(new_vehicle))

    def start_all_vehicles(self):
        """모든 교통 수단을 주행 시작시킨다"""
        print("모든 교통 수단을 주행 시작시킵니다!\n")
        return [new_vehicle.start() for new_vehicle in self.new_vehicles]

    def stop_all_vehicles(self):
        """모든 교통 수단을 주행 정지시킨다"""
        print("모든 교통 수단을 주행 정지시킵니다!\n")
        return [new_vehicle.stop() for new_vehicle in self.new_vehicles]

    def __str__(self):
        """갖고 있는 교통 수단들의 현재 속도를 문자열로 리턴한다"""
        #return [new_vehicle.__str__() for new_vehicle in self.new_vehicles]
        #speed_info = str(Bicycle()) + "\n" + str(NormalCar()) + "\n" + str(SportsCar())
        #print(speed_info)

        res_str = ""
        for new_vehicle in self.new_vehicles:
            res_str += str(new_vehicle) + "\n"
        return res_str
